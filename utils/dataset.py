...
class LongSeqDataset(Dataset):
    ...
    def __getitem__(self, i):
        if i % 30 == 0:
            imgu2 = np.array(Image.open(self.img_file_names[i]))
            imgu1 = np.array(Image.open(self.img_file_names[i]))
        elif i % 30 == 1:
            imgu2 = np.array(Image.open(self.img_file_names[i - 1]))
            imgu1 = np.array(Image.open(self.img_file_names[i - 1]))
        else:
            imgu2 = np.array(Image.open(self.img_file_names[i - 2]))
            imgu1 = np.array(Image.open(self.img_file_names[i - 1]))
        img = np.array(Image.open(self.img_file_names[i]))
        if i % 30 == 29:
            imgd1 = np.array(Image.open(self.img_file_names[i]))
            imgd2 = np.array(Image.open(self.img_file_names[i]))
        elif i % 30 == 28:
            imgd1 = np.array(Image.open(self.img_file_names[i + 1]))
            imgd2 = np.array(Image.open(self.img_file_names[i + 1]))
        else:
            imgd1 = np.array(Image.open(self.img_file_names[i + 1]))
            imgd2 = np.array(Image.open(self.img_file_names[i + 2]))
        mask = np.load(self.mask_file_names[i])

        pad = lambda img: np.stack([img,img,img],axis=2) if (len(img.shape)==2 or img.shape[2]==1) else img
        imgu2, imgu1, img, imgd1, imgd2 = pad(imgu2), pad(imgu1), pad(img), pad(imgd1), pad(imgd2)

        imgu2, imgu1, img, imgd1, imgd2 = self.input_transform(imgu2), self.input_transform(imgu1), self.input_transform(img), self.input_transform(imgd1), self.input_transform(imgd2)
        mask = self.target_transform(mask)

        return imgu2, imgu1, img, imgd1, imgd2, mask
...
class LongSeqTestDataset(Dataset):
    def __init__(self, img_dir, mask_dir, input_transform, target_transform, logger=None):
        self.img_dir = np.load(img_dir)
        self.mask_dir = mask_dir
        self.img_file_names = [join('data/imgs', file_name) for file_name in self.img_dir if is_image_file(file_name)]
    ...
    def __getitem__(self, i):
        ...
        return imgu2, imgu1, img, imgd1, imgd2, mask, self.img_file_names[i]
